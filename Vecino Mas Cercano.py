...
def vecino_mas_cercano(problema: dict, disponibles: list): 
    ...
    #Recorre por cada elemento en el diccionario
    for key in problema: 
        #Obtiene los nodos del diccionario
        disponibles = [i for i in problema]
        #Asigna nodo a al nodo actual acorde a la iteracion
        nodo_actual = key
        #Añade a ruta el nodo correspondiente 
        ruta = [key]
        #Remueve los demas nodos
        disponibles.remove(key)

        #Mientra haya vecinos
        while len(disponibles) > 0: 
            vecino_mas_cercano = 0
            dist = inf
            #Recorre todos los nodos en la ruta disponible
            for v in disponibles: 
                if v != nodo_actual: 
                    #Calcula la distancia entre nodo actual y el iterado
                    if problema[nodo_actual][v] <= dist: 
                        dist = problema[nodo_actual][v]
                        vecino_mas_cercano = v
                        #Añade el nodo mas cercano a la ruta
            ruta.append(vecino_mas_cercano)
            #Remueve los anteriores
            disponibles.remove(vecino_mas_cercano)
        ruta.append(key)
        print(str(ruta))
        #Calcula la distancia entre estos nodos para comprobar si es la solucion factible
...
if __name__=="__main__": 
    ...
    #Iniciamos contador de solucion
    t = time()
    
    #Obtenemos ruta a traves de la funcion vecino, mandando como parametro el diccionario y la lista
